...
type EditProps = {
  posts: typePost[];
  abrirModal: (value: null | number) => void;
  editedPost: number | null;
};

export default function EditPost({ posts, abrirModal, editedPost }: EditProps) {
  const thisPost = posts.find((post) => post.idPost == editedPost);

  const [desc, setDesc] = useState(thisPost?.desc || "");
  const [image, setImage] = useState<File | null>(null);
  const [imageUrl, setImageUrl] = useState<string>(thisPost?.imagem || "");
  const [loc, setLoc] = useState(thisPost?.loc || "");
  const [type, setType] = useState(thisPost?.typePos || "");
  const [status, setStatus] = useState(thisPost?.status || "");
  const [message, setMessage] = useState("");

  const [haveStatus, setHaveStatus] = useState(thisPost?.typePos === "ProjetoSocial" || thisPost?.typePos  === "Denuncia");
